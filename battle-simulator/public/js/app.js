...
function library() {
  const curated = Object.entries(CURATED).map(([id, raw]) => normalizeBattle(raw, { id, source: "curated", people: PEOPLE }));
...
const TABS = [
  ["context", "I. The Context"],
  ["simulator", "II. The Battle"],
  ["aftermath", "III. The Aftermath"],
];
...
function renderSimulator(body, b, phaseNum) {
  ...
  const show = (i, { instant = false, fromUser = true } = {}) => {
    ...
    body.querySelector(".plate-n").textContent = `Plate ${toRoman(index + 1)}`;
...
function figureCard(b, f) {
  ...
  return `
    <article class="figure" style="--side:${side?.color || "var(--ink-3)"}">
      ...
      ${others.length ? `<p class="figure-also">Also in the atlas: ${others.sort((x, y) => x.year - y.year).map((o) => `<a href="#/battle/${encodeURIComponent(o.id)}">${esc(o.name.replace(/^Battle of (the )?/, ""))}</a>`).join(", ")}</p>` : ""}
    </article>`;
}
...
function nativeLang(t) {
  return /[\u3400-\u9fff]/.test(t) ? "zh-Hant" : /[\u0370-\u03ff]/.test(t) ? "el" : /[\u0400-\u04ff]/.test(t) ? "ru" : "";
}
